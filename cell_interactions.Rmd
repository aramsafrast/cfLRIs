...
lc_sc %>%
  ...
  as.matrix() %>%
  chordDiagramFromMatrix(link.lty = lty_df, link.lwd = lwd_df, link.border = border_df, direction.type = "arrows", directional = 1, grid.col = grid.col, link.sort = TRUE)
...
lc_sc %>%
  ...
  `rownames<-`(paste0(rownames(.), " ")) %>%
  as.matrix() %>%
  chordDiagramFromMatrix(link.lty = lty_df, link.lwd = lwd_df, link.border = border_df, direction.type = "arrows", directional = 1, grid.col = grid.col, link.sort = TRUE, scale = F)
...
lc_sc %>%
  ...
  `rownames<-`(paste0(rownames(.), " ")) %>%
  as.matrix() %>%
  chordDiagramFromMatrix(link.lty = lty_df, link.lwd = lwd_df, link.border = border_df, direction.type = "arrows", directional = 1, grid.col = grid.col, link.sort = TRUE, scale = T)
